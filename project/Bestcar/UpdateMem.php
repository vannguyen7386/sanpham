	...
	if (isset($_POST['ChangePass'])){
		...
		if($result){
			if(isset($_POST['NewPass']) and strlen($_POST['NewPass']) > 4 ){
				...
				header("location:index.php?page=MemDoc&success=Đổi mật khẩu thành công");
			}else{
		...
		if($result){
			if(isset($_POST['NewEmail'])){
				...
				header("location:index.php?page=MemDoc&success=Đổi email thành công");
			}else{
				header("location:index.php?page=Change&field=email&err=Email không đúng");
			}
		}else{
			header("location:index.php?page=Change&field=email&err=Email hoặc mật khẩu không đúng");	
		}
		...
		header("location:index.php?page=MemDoc&success=Cập nhật thành công");
	}else{
