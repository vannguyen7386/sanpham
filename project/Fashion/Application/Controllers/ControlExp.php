<?php
    ...
    if(!isset($_REQUEST['Id'])){
        $query = "SELECT COUNT(*) FROM fas_extra_info WHERE ModuleID=3";
        $pageNum = $pagging->get_Page($query);
        $display = $pagging->get_display();
        $start = $pagging->get_start();
       
        
        $sql = "SELECT *,DATE_FORMAT(InfoDate,'%d/%m/%Y - %r') AS datetime FROM fas_extra_info WHERE ModuleID=3 ORDER BY InfoDate DESC  LIMIT {$start},{$display}";
    ...
    include'Application/Views/ViewExp.php';
?>